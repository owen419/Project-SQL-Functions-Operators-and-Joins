...
select c.first_name, c.last_name, c.customer_id, p.price
from sales s
...
select c.first_name, c.last_name, m.first_name, m.last_name, c.membership_status
from customers c
inner join customers m
on c.membership_status = m.membership_status
and c.customer_id < m.customer_id
order by c.customer_id, c.membership_status, m.customer_id;

-- 18. Write a query to join the `Sales` and `Products` tables, and calculate the total number of sales for each product.
select s.product_id, p.product_name, count(*) as number_of_transactions,
sum(s.quantity_sold)as total_number_of_sales
from products p
join sales s
on p.product_id = s.product_id
group by s.product_id, p.product_name, s.quantity_sold
...
select s.product_id, p.product_name, sum(s.quantity_sold) as total_number_of_sales
from products p
join sales s
on p.product_id = s.product_id
where s.quantity_sold > 5
group by s.product_id, s.quantity_sold, p.product_name;
...
select c.first_name, c.last_name, p.category, p.product_name
from sales s
join products p
on p.product_id = s.product_id
join customers c
...
select c.customer_id, c.first_name, c.last_name, s.sale_date, sum(s.total_amount) as total_sales
from sales s
right join customers c
on c.customer_id =s.customer_id
where sale_date between '2023-01-01' and '2023-12-31'
group by c.customer_id, c.first_name, c.last_name, s.sale_date, s.total_amount
...
right join products p
on s.product_id = p.product_id
group by p.product_name, p.price, p.product_id, s.sale_id
...
select sum(s.quantity_sold), c.membership_status
from sales s
join customers c
on s.customer_id = c.customer_id
join products p
...
select c.customer_id, c.first_name, c.last_name, s.quantity_sold as quantity_bought
from sales s
join products p
on s.product_id = p.product_id
join customers c
on s.customer_id = c.customer_id
where s.quantity_sold > 5
group by c.customer_id, c.first_name, c.last_name, s.quantity_sold;

-- 30. Write a query to find the average quantity sold per product.
select round(avg(s.quantity_sold) / p.product_id ,2) as avg_quantity_sold_per_product, p.product_name
from sales s
join products p
on s.product_id = p.product_id
...
on s.customer_id = c.customer_id
where sale_date - registration_date <= 7;
...
select c.first_name, c.last_name, sum(s.quantity_sold) / count(c.customer_id) as quantity_sold_per_customer
from sales s
join customers c
on s.customer_id = c.customer_id
...
(select product_id, product_name, stock_quantity, 'Highest Stock' as category
from products
order by stock_quantity desc
limit 1)
union
(select product_id, product_name, stock_quantity, 'Lowest Stock' as category
from products
...
select c.first_name, c.last_name, sum(s.total_amount) as total_sales_amount, p.product_name
from sales s
inner join customers c
on s.customer_id = c.customer_id
join products p
...
right join products p
on s.product_id = p.product_id
group by p.category, s.quantity_sold
order by quantity_sold desc;
...
select p.product_name, sum(s.quantity_sold) as total_sales, extract(month from s.sale_date) as month, extract(year from s.sale_date) as year
from sales s
right join products p
on s.product_id = p.product_id
group by p.product_name, s.sale_date, extract(month from s.sale_date), extract(year from s.sale_date)
...
select s.product_id, sum(s.quantity_sold), i.stock_quantity
from sales s
...
select count(distinct product_id) as total_number_of_unique_products
from sales
where sale_date between '2023-01-01' and '2023-12-31';
...
join products p
on p.product_id = s.product_id
where sale_date <= date '2024-02-05' - interval '6 months'
...
select c.first_name, c.last_name, s.sale_date, sum(s.total_amount) as most_money_spent
from sales s
right join customers c
on s.customer_id = c.customer_id
where sale_date between '2023-01-01' and '2023-12-31'
...
select p.product_id, p.product_name, s.quantity_sold, p.price
from sales s
left join products p
on s.product_id = p.product_id
where s.quantity_sold > 100
and p.price > 200
group by p.product_id, p.product_name, s.quantity_sold;
